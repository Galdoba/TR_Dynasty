...
//CheckTask - rolls check task and returns effect of the check
//effect >= 0 is considered successful
func (cei *CEI) CheckTask(difficulty int, addMods ...int) int {
	r := dice.Roll2D() + cei.TaskDM()
...
func (cei *CEI) ResolveTask(addMods ...int) int {
	r := dice.Roll2D() + cei.TaskDM()
	for _, mod := range addMods {
		r += mod
	}
